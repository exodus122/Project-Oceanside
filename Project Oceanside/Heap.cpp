...
#include "Room.h"
#include "Constants.h"
#include "Heap_SolveMM.h"
#include "Heap_SolveOoT3D.h"

// Constructor and Destructor
Heap::Heap(Scene* scene, char version, int start, int linkSize) : start_address(start), scene(scene), linkSize(linkSize)
{
...
void Heap::Allocate(Node* node)
{
	...
	if ((version != OOT_3D) && (node->GetOverlay() != nullptr && currentActorCount[node->GetID()] == 0))
	{
	...
	else
	{
		...
		//std::cout << std::hex << suitableGap->GetAddress() << '\n';
...
void Heap::ChangeRoom(int newRoomNumber, int transitionActorID, Node* carryActor, bool spawners)
{
	...
	if (carryActor)
	{
		AllocateNewRoom(*newRoom, *oldRoom, transitionActorID);
		DeallocateClearedActors();
		UnloadRoom(*oldRoom, transitionActorID, carryActor);
	...
	else
	{
		AllocateNewRoom(*newRoom, *oldRoom, transitionActorID);
		DeallocateClearedActors();
		UnloadRoom(*oldRoom, transitionActorID, nullptr);
...
void Heap::UnloadRoom(Room& room, int transitionActorID, Node* carryActor)
{
	...
	{
		if (actor.second->IsTransitionActor() && actor.second->GetSceneTransitionID() != transitionActorID)
		{
...
void Heap::AllocateNewRoom(Room& newRoom, Room& oldRoom, int transitionActorID)
{
	if (transitionActorID > scene->NumberOfTransitionActors() - 1)
	{
	...
	for (Node* actor : newRoom.GetAllActors())
	{
		if (actor->IsTransitionActor())
		{
			...
			if (currentActorSceneID == transitionActorID)
			{
...
void Heap::PrintHeap(char setting) const
{
	Node* curr = head;
	if (!setting) {
		std::cout << "-----HEAP OUTPUT----- Room: " << currentRoomNumber <<  std::endl;
		
	}
	else {
		std::cout << "-----HEAP OUTPUT With Links----- Room: " << currentRoomNumber << std::endl;;
	}
	while (curr != nullptr)
	{
		if (setting == 0)
...
{
	std::cout << "Printing current actor count" << std::endl;
	for (auto i : currentActorCount)
	{
		std::cout << "actor id = " << std::hex << i.first << ", count = " << i.second << std::endl;
	}
}
